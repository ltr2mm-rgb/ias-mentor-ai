...
# This module used to build the client at IMPORT time:
#     client = genai.Client(api_key=GEMINI_API_KEY)
# GEMINI_API_KEY has no default (config.py), so with the variable unset it was None,
# and any failure constructing the client raised during import. main.py imports this
# module at module level, and ALL THREE provider slots (gemini/openai/deepseek) are
# registered from here — so one missing key did not merely disable Gemini, it
# unregistered every provider and prevented the whole application from starting:
# no login, no dashboard, no admin tools.
#
# The client is now built on first use, mirroring deepseek_service._get_client()
# (the pattern already established in this repo). A missing or unusable key now
# degrades AI features instead of taking the product offline, and the reason is
# recorded in LAST_GEMINI_ERROR so diagnostics can report it rather than it failing
# silently.
_client = None
LAST_GEMINI_ERROR = ""
...
def _get_client():
    """The Gemini client, or None if it cannot be built. Never raises."""
    global _client, LAST_GEMINI_ERROR
    if _client is None:
        if not GEMINI_API_KEY:
            LAST_GEMINI_ERROR = "GEMINI_API_KEY is not set"
            return None
        try:
            ...
            _client = None
            LAST_GEMINI_ERROR = str(e)[:300]
    return _client


def _require_client():
    """Client for a call that cannot proceed without one. Raises a clear, catchable
    error that ai_runtime treats as a provider failure and falls back from."""
    c = _get_client()
    if c is None:
        raise RuntimeError("Gemini unavailable: " + (LAST_GEMINI_ERROR or "no client"))
    return c
...
# Google retires model IDs over time (gemini-2.5-flash-lite became unavailable to
# NEW users in 2025 → 404 NOT_FOUND). We keep an ordered candidate list per tier and
# use the first that works, caching it, so a retirement auto-falls to the next model
# instead of breaking all generation. Env vars still force a specific id.
MODEL = _model_os.getenv("GEMINI_MODEL", "gemini-2.0-flash")
# High-stakes reasoning tier: a stronger Gemini used SPARINGLY for the few tasks
# where judgement matters most (Mains evaluation, mistake diagnosis, interview gen).
MODEL_PRO = _model_os.getenv("GEMINI_PRO_MODEL", "gemini-2.5-pro")
_MODEL_CANDIDATES = [m for m in [
    _model_os.getenv("GEMINI_MODEL", "").strip(),
    "gemini-2.0-flash", "gemini-flash-latest", "gemini-2.5-flash",
    "gemini-2.0-flash-lite", "gemini-1.5-flash",
] if m]
_PRO_CANDIDATES = [m for m in [
    _model_os.getenv("GEMINI_PRO_MODEL", "").strip(),
    "gemini-2.5-pro", "gemini-pro-latest", "gemini-1.5-pro", "gemini-2.0-flash",
] if m]
_RESOLVED_MODEL = {}   # tier -> working model id (cached after first success)
...
def _gemini_ok() -> bool:
    # Also require a usable client: without one, ai_runtime should skip Gemini and
    # fall through to the next provider rather than attempt a call that must fail.
    return _gtime.time() >= _GEMINI_DOWN_UNTIL and _get_client() is not None
...
def _gemini_raw(prompt: str, json_mode: bool, model: str) -> str:
    _c = _require_client()
    if json_mode:
        return (_c.models.generate_content(
            model=model, contents=prompt,
            config={"response_mime_type": "application/json"}).text or "")
    return (_c.models.generate_content(model=model, contents=prompt).text or "")


def _gemini_text(prompt: str, json_mode: bool = False, model: str = None) -> str:
    """Generate via Gemini, resilient to retired model IDs: try the requested model,
    then the cached working model, then the tier's candidate list — on a NOT_FOUND /
    'no longer available' / 'unsupported' error, retire that id and try the next.
    Availability errors (quota/billing) bubble up so the runtime falls back engines."""
    tier = "pro" if (model and "pro" in model.lower()) else "flash"
    cands = _PRO_CANDIDATES if tier == "pro" else _MODEL_CANDIDATES
    order, seen = [], set()
    for m in ([model] if model else []) + \
             ([_RESOLVED_MODEL.get(tier)] if _RESOLVED_MODEL.get(tier) else []) + cands:
        if m and m not in seen:
            seen.add(m); order.append(m)
    last = None
    for m in order:
        try:
            out = _gemini_raw(prompt, json_mode, m)
            _RESOLVED_MODEL[tier] = m          # cache the working id for this tier
            return out
        except Exception as e:
            last = e
            s = str(e).lower()
            if ("not_found" in s or "not found" in s or "no longer available" in s
                    or "404" in s or "is not supported" in s or "unsupported" in s):
                continue                        # retire this id, try the next candidate
            raise                               # quota/billing/etc → bubble to gen_text
    if last:
        raise last
    return ""


PROVIDER_PRIORITY = ["gemini", "openai", "deepseek"]  # legacy alias; see ai_runtime.DEFAULT_PRIORITY

# ── Register providers into the AI Execution Runtime (architecture §9) ─────────
# The runtime is the single chokepoint for every LLM call. Nothing above it imports
# a model SDK — it only knows these adapters. gen_text (and, over time, every
# caller) routes through ai_runtime, which owns provider order, per-call timeout,
# output validation, fallback and decision logging.
import ai_runtime


def _rt_gemini(prompt, json_mode=False, system=None, model_tier="flash"):
    full = (system + "\n\n" + prompt) if system else prompt
    m = MODEL_PRO if model_tier == "pro" else MODEL
    try:
        return _gemini_text(full, json_mode=json_mode, model=m)
    except Exception as e:
        _note_gemini_error(e)          # feed the quota circuit-breaker
        raise


def _rt_openai(prompt, json_mode=False, system=None, model_tier="flash"):
    return _openai_generate(prompt, json_mode=json_mode, system=system)


def _rt_deepseek(prompt, json_mode=False, system=None, model_tier="flash"):
    return _ds.ds_generate(prompt, json_mode=json_mode, system=system) if _ds else ""


ai_runtime.register_provider("gemini", _rt_gemini, _gemini_ok)
ai_runtime.register_provider("openai", _rt_openai, _openai_on)
ai_runtime.register_provider("deepseek", _rt_deepseek, _deepseek_on)


def gen_text(prompt: str, json_mode: bool = False, prefer: str = "gemini",
             system: str = None, model: str = None, task: str = "text.generic") -> str:
    """Resilient text generation — now a thin wrapper over the AI Execution Runtime
    (`ai_runtime.route`). Behaviour is preserved: priority Gemini → OpenAI → DeepSeek,
    `prefer` moves one engine to the front (e.g. cross-model verification), Gemini is
    skipped during its quota cooldown, and `model` forces the Gemini tier
    (MODEL_PRO → 'pro'). Callers may pass an explicit `task` to use a registered
    routing policy; default is 'text.generic'. Returns the raw string, as before."""
    tier = None
    if model is not None:
        tier = "pro" if model == MODEL_PRO else "flash"
    return ai_runtime.route(task=task, prompt=prompt, system=system,
                            json_mode=json_mode, model=tier, prefer=prefer).output
    ...
    for model in ([EMBED_MODEL] + [m for m in EMBED_MODELS if m != EMBED_MODEL]):
        for cfg_mode in ("dim", "task", "none"):
            try:
                ...
                _c = _require_client()
                if cfg is not None:
                    resp = _c.models.embed_content(model=model, contents=contents, config=cfg)
                else:
                    resp = _c.models.embed_content(model=model, contents=contents)
...
def chat_with_mentor(message: str, context: str = "", history=None) -> str:
    ...
    persona = """You are MEERA — an AI-based IAS (UPSC) mentor who is, all at once, the student's
...
- You only discuss UPSC preparation, study, motivation and the student's journey. Politely redirect off-topic asks."""
    ...
    for m in (history or [])[-10:]:
        who = "Student" if m.get("role") == "user" else "MEERA"
        convo += f"\n{who}: {m.get('content','')}"
...
Now respond to the student's latest message as MEERA — personal, helpful, and grounded in what you know.
...
def ocr_image(img_bytes: bytes, mime_type: str = "image/png") -> str:
    ...
    if genai_types is not None:
        try:
            resp = _require_client().models.generate_content(
                model=MODEL,
                contents=[
