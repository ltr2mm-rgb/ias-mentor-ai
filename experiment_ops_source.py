...
def _load_active_experiments():
    """(featured Experiment | None, [history Experiments]) from an optional
    `active_experiments.json`. Only known Experiment fields are accepted; unknown
    keys are ignored so a config typo can't crash the endpoint."""
    import experiment as ex
    if not os.path.exists(ACTIVE_PATH):
        return None, []
    with open(ACTIVE_PATH, "r", encoding="utf-8") as f:
        cfg = json.load(f)
    fields = {fld.name for fld in dataclasses.fields(ex.Experiment)}

    def mk(d: Dict[str, Any]):
